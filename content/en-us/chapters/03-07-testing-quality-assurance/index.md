# Chapter 3.7 — Testing & Quality Assurance
...
2. **Treat regression testing as a clinical safety control.** Before any change reaches a live clinical system, an automated regression suite must confirm that existing, safety-relevant behavior still works. Map your highest hazards from the DCB0129/0160 hazard log directly to named regression tests, so a change that would reintroduce a known hazard fails the build automatically. This is the mechanism that lets you iterate on live systems without accumulating silent clinical risk.

3. **Never test with live patient data by default.** Provision test and staging environments with synthetic or robustly anonymized data, governed by the same information governance and DPIA discipline as production (see Chapter 1.7 — Information Governance, Data Protection & Cyber Security). If a specific test genuinely requires production-like data, treat it as a controlled exception with a documented lawful basis, minimization, and audit — not as routine convenience. A test database is still a patient database.
...
9. **Integrate testing into the delivery pipeline, not after it.** Wire automated tests, security scanning, and accessibility checks into the continuous integration and delivery pipeline so every change is tested before it can progress (see Chapter 3.5 — Delivery Lifecycles). This makes quality continuous and cheap, and it means the ability to deploy a tested fix quickly — itself a safety control — is always available. Manual, end-of-project test phases are slow, late, and the first thing sacrificed when time runs short.
...
1. **If we change one thing in our live clinical system tomorrow, how confident are we that we would find out — before a patient does — if it broke something else?**
   This question probes the maturity of your regression testing and the honesty of your safety claims. Most serious clinical software incidents are not exotic new failures but regressions: a change that quietly broke behavior everyone assumed was stable, discovered only when a clinician or patient hit it. A good discussion traces a recent real change through to the tests that would have caught a regression, and asks whether the highest hazards in the DCB0129/0160 hazard log are each backed by an automated test that fails when the hazard reappears. The tension to surface is between the pressure to ship fixes fast and the discipline of proving nothing else broke — and whether your pipeline makes that proof automatic or leaves it to someone remembering to check. Consider how long your test suite takes to run, whether it runs on every change, and whether anyone has quietly disabled failing tests to hit a deadline. An honest answer distinguishes "we have a regression suite tied to our clinical hazards and trust it" from "we test the change we made and hope for the best". If the answer is the latter, you are carrying clinical risk you cannot see.

2. **Where does our test data come from, and could a test ever expose a real patient's information?**
   Test environments are a common and under-examined route to a data breach, because teams under pressure reach for the easiest realistic data — a copy of production — and the resulting patient records then sit in less-protected test systems, laptops, or supplier environments. This question asks the team to trace every test and staging environment back to its data source, and to confront honestly whether "just this once" copies of live data have become routine. The trade-off to debate is realism versus safety: synthetic and anonymized data are safer but can miss the messy edge cases real data contains, while live data is realistic but a governance and breach liability, and true anonymization is harder than it looks because records can be re-identified when combined. Concrete angles include who can access test environments, whether test databases are encrypted and audited like production, and whether your suppliers' test environments meet the same bar. A strong answer describes a deliberate strategy — synthetic data by default, anonymization done properly, live data only as a documented, minimized, time-limited exception — rather than an accident waiting for an information governance investigation.

3. **When the go-live date and the quality bar collide, what actually gives — and who decides?**
   Every delivery eventually faces the moment when testing is not finished, a defect is unresolved, or accessibility conformance is incomplete, and the launch date is looming; how your organization behaves in that moment reveals whether quality is a genuine control or merely aspirational. This question asks the team to recall the last time it happened and describe honestly what was cut: was it the cosmetic backlog, or was it regression testing, UAT, or the accessibility audit? The tension is real and should not be waved away — dates matter, funding cycles are unforgiving, and endless testing can itself be a failure mode — but the resolution must be governed by clinical and equity risk, not by whoever shouts loudest. Discuss who has the authority to hold a launch on quality grounds without career risk, whether your definition of done is ever formally waived and by whom, and whether known defects shipped at go-live are recorded as accepted risks or quietly buried. A good answer shows a clear, named decision route, a bright line below which a defect blocks release regardless of date (anything that could contribute to patient harm), and a culture where saying "not yet, it isn't safe" is rewarded rather than punished.

4. **Are our tests actually covering the things that could harm a patient, or just the things that are easy to test?**
   A green build is reassuring only if the tests behind it exercise the real clinical hazards, and it is uncomfortably common for a healthy-looking coverage figure to sit on top of a suite that checks trivial getters and setters while the deterioration score, the identity match, and the allergy display go untested. This question asks the team to hold the test suite up against the DCB0129/0160 hazard log and the highest-risk user journeys and ask, hazard by hazard, "which named test would fail if this went wrong?" The tension is that coverage metrics reward volume, not risk relevance, and teams are tempted to chase a percentage rather than confront the awkward, hard-to-automate cases — boundary values, missing inputs, unusual clinical combinations — that are exactly where patients get hurt. Concrete angles include reviewing whether boundary and edge cases are tested, whether exploratory and clinical testing (the judgment machines cannot replace) has a place alongside automation, and whether anyone has ever traced a real incident back to a gap the tests should have caught. An honest answer talks about risk-weighted coverage and can name the tests guarding the top hazards, rather than quoting a single percentage as if it were assurance.

5. **When we test for accessibility, are real disabled users and real clinicians in the room, or just an automated scanner?**
   Automated WCAG checks are cheap and belong on every build, but they detect only a fraction of the barriers that actually exclude people, and a service can pass every automated rule while remaining unusable with a screen reader or by keyboard alone. This question asks whether your accessibility and user acceptance testing reach real people using assistive technology and real clinicians working through genuine workflows, or whether "we tested accessibility" quietly means "the scanner was green." The trade-off to surface is cost and logistics versus honesty: recruiting assistive-technology users and backfilling clinical time for realistic UAT is expensive and easy to skip, yet skipping it is how a legal duty is breached and how a dangerous workflow mismatch — a warning at the wrong moment, a default that invites error — reaches a live ward. Discuss who you involve, how representative they are, whether testing happens in a setting that mirrors the clinic rather than a quiet meeting room, and what you do with what they find. A strong answer describes automated checks paired with manual testing by disabled users and clinician-led UAT as standing, budgeted practice, not a one-off box ticked before go-live.

6. **When a change crosses a boundary between our system and someone else's, who tests the join — and would we know if it broke?**
   Modern health and care services are assemblies of integrated systems — an EPR, a national spine service, a pathology feed, a third-party supplier's module — and the most dangerous defects often live not inside any one component but in the seams between them, where a change on one side of an interface silently breaks the other. This question asks the team to map its critical integrations and ask, for each, who owns the end-to-end test, what synthetic data flows across the boundary, and how a breaking change in a supplier's release would be detected before it reached patients. The tension is that no single team feels wholly responsible for a shared interface, contracts and change advisory boards move at different speeds, and testing across organizational boundaries is slow, political, and easy to leave to chance. Concrete angles include contract and interface tests, shared test environments, how supplier change notifications are handled, and whether your regression suite exercises the integrated journey (referral sent, received, acknowledged) and not just your own half of it. An honest answer names who is accountable for each join and shows a mechanism — automated where possible — that catches a cross-boundary regression, rather than assuming the other party has it covered.
...
An established small provider — a GP practice, a community pharmacy, a care home, or a single-site clinic — usually buys its clinical systems rather than building them, so its testing job is acceptance, not construction: confirming that a supplier's update or a new integration still works safely in the real workflow before staff depend on it. With little or no dedicated IT capability, the practical discipline is a short, repeatable checklist run against the highest-risk journeys (booking, prescribing, recording observations) after every supplier change, ideally in a test or training environment with synthetic patients rather than live records. The same DCB0160 deployment-safety and accessibility duties apply as to a large trust, but without the test team, so the realistic move is to lean on the supplier's assurance while retaining the handful of local acceptance checks only you can do — does it fit how this clinic actually works? A small provider that skips even this and tests changes directly on live patients is one bad update away from a safety incident it has no way to catch.
...
- **Live data in test environments.** Real patient records are copied into less-protected test systems for realism, creating a breach waiting to happen. Antidote: synthetic and anonymized data by default; live data only as a governed exception.
...
|---|---|---|---|---|---|
| Test strategy | Ad-hoc manual testing before release | Some automated tests; inverted pyramid | Test pyramid with automation across levels | Coverage and pass rates tracked against targets; the strategy is measured and actively managed | Shift-left culture; fast, trusted feedback on every change |
| Regression & safety link | No systematic regression testing | Manual regression cycles, incomplete | Automated regression suite mapped to hazard log | Hazard-to-test coverage measured and assured; gaps flagged and closed against a target | Hazard-linked tests gate every release automatically |
| Test data | Live patient data copied casually | Mix of live and anonymized, ungoverned | Synthetic/anonymized by default, governed | Test-data provenance and access audited and reported; exceptions tracked and time-limited | Robust synthetic data with realistic edge cases; live data never used |
| Non-functional testing | None or afterthought | Occasional load/security checks | Performance, security, accessibility tested routinely | Non-functional results measured against defined thresholds and controls assured | Continuous non-functional testing at realistic peak scale |
| Defects & done | No defined "done"; defects untracked | "Done" defined but often waived | Enforced gates; defects triaged by clinical severity | Defect trends and gate compliance tracked as KPIs and managed to target | Quality built in; releases boring and reversible |
...
- [ ] Test and staging environments use synthetic or anonymized data; live patient data is never used casually.
