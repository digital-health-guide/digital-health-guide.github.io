# Chapter 3.8 — Service Management & Live Operations
...
**Run-versus-build and decommissioning.** Organizations separate *build* funding (creating new capability) from *run* funding (operating what exists). Underfunding run is the structural root of most operational failure. And every service eventually reaches *decommissioning* — safe retirement, with data migrated or archived under retention law and users redirected — which is an operational discipline in its own right.
...
3. **Set SLOs with clinicians and manage an error budget.** Define what "reliable enough" means for each service as measurable SLIs and SLOs, and set them jointly with the clinicians who bear the consequences — availability targets for an emergency-department system are not a purely technical choice. Use the error budget to arbitrate the eternal tension between shipping features and staying stable: when reliability is spent, stop and stabilize. Keep external SLAs looser than internal SLOs so you have room to act before you breach a promise.
...
3. **How do we decide what "reliable enough" means — and who gets a say?**
   Reliability is not free, and chasing an extra nine of availability can cost more than the harm it prevents; but in clinical systems, "acceptable" downtime is a safety judgment that engineers cannot make alone. This question asks whether you have explicit SLIs and SLOs, whether they were set with the clinicians and operational leaders who bear the consequences, and whether you use something like an error budget to balance shipping improvements against staying stable. Debate the difference between the internal target you manage to and the external SLA you promise, and whether your monitoring measures what clinicians actually experience or just what servers report. Consider whether "we aim for 99.9%" has any teeth — a defined measurement, a consequence when breached, an owner who acts. A good answer connects a reliability target to a clinical-safety rationale, shows the metric is measured and owned, and describes what the team does differently when the budget is spent, rather than a number in a slide no one manages to.

4. **Do we actually know what we run — and could we answer "what will this change affect?" in the middle of an incident?**
   Most large health and care estates are weakest exactly where it hurts most: no one holds an accurate, current picture of what runs, what depends on what, which versions are deployed, and who owns each service. This question probes whether you maintain a configuration record — a CMDB or equivalent — or whether that knowledge lives in a handful of people's heads and a decade of undocumented integrations. Debate the real consequences: a change that quietly breaks a downstream system, a security patch that cannot be applied because no one knows what it will touch, an unsupported server discovered only when it fails or is exploited. The tension is that configuration management is unglamorous, never finished, and easy to defer, yet it underpins safe change, patching, licensing, and cyber-resilience alike — the same blind spot that turned WannaCry into a crisis. An honest answer describes a maintained inventory with owners and dependencies that people trust and use during incidents, and names the parts of the estate that are still dark, rather than claiming a completeness no large organization truly has.

5. **When something goes wrong, do we learn or do we look for someone to blame?**
   The culture of your incident reviews determines whether the same harm recurs. This question asks whether serious incidents trigger a blameless, systemic review that asks how the system allowed the failure — or whether they quietly hunt for an individual to fault, which drives reporting underground and guarantees repetition. Debate whether near-misses and small failures get surfaced at all, or only the outages too big to hide; whether review actions are assigned owners and tracked to closure, or written down and forgotten; and whether patterns feed back into problem management and design. Consider the parallel with clinical incident reporting and "just culture": the same organization may hold a mature, blameless stance for patient-safety incidents yet a punitive one for IT failures, or vice versa. A good answer shows a learning culture with evidence — reviews that changed the system, actions closed, recurrence falling — and an honest acknowledgment of where fear still suppresses reporting, because a learning culture is itself a patient-safety control, not a soft extra.

6. **Have we decided how this service will eventually be retired — while it is still alive to plan it?**
   Every service is eventually replaced or switched off, yet decommissioning is almost always an afterthought, scrambled when the successor arrives and the old system is already an obstacle. This question asks whether, for each live service, you have a plan for safe retirement: data migrated or archived under records-retention law, users redirected, clinicians notified, and the hazard log and audit trail preserved. Debate the specific health and care risks of getting this wrong — stranded records that breach statutory retention duties, orphaned data no one can access, a "temporary" legacy system that lingers for years unpatched because no one owns its ending. Consider that in national and enterprise estates, decades of clinical records may sit inside a system that must be decommissioned without losing a single legally required record. An honest answer treats retirement as a named, funded, auditable operation planned from go-live, not a cost to be discovered later, and can point to at least one service whose end-of-life is already decided rather than deferred.

## In practice: a health & care example

An integrated care system (ICS) runs a shared care record used by hospital clinicians, GPs, community teams, and out-of-hours services across a region. It went live two years ago; the build program has closed and a standing product-and-operations team now owns it. That team learned service management the hard way.

Early on, the record had monitoring only of server uptime. One winter evening, the servers were "green" while a failing integration silently stopped GP medication updates from flowing through — so hospital clinicians saw stale medication lists for several hours before a pharmacist noticed and phoned the service desk. The post-incident review was blameless and systemic: it found the real problem was the absence of an SLI reflecting the clinician's experience. The team added end-to-end observability, an SLO for data freshness agreed jointly with a clinical safety officer, and an alert that pages on-call when updates lag beyond the safety threshold. This hazard was added to the clinical risk hazard log (Chapter 1.6).

They then right-sized their practices. Routine, low-risk configuration updates became standard pre-approved changes shipped through an automated pipeline with fast rollback; only genuinely risky changes go to a weekly change review with clinical representation. A humane on-call rota covers nights and holidays, backed by a service desk resourced to the real 24/7 demand. Crucially, they wrote and now twice-yearly rehearse a clinical downtime procedure: a read-only cached copy of key records, a paper fallback, an SMS cascade to tell clinicians the system is degraded, and a reconciliation step on recovery. When a data-center power fault took the primary system offline for forty minutes last spring, care continued on the tested fallback, the incident commander ran a calm coordinated response, and no patient was harmed. The board saw a reliability report tied to clinical-safety measures, not a vague reassurance — and the service's ownership through to eventual decommissioning is named and funded.
...
An established small provider — a GP practice, a community pharmacy, a care home, or a single-site clinic — runs live clinical systems every day but usually has no dedicated IT function and no one whose job is service management. Here "run" is squeezed into the margins of clinical and administrative work, on-call is really "the practice manager's mobile", and monitoring is a clinician noticing the booking screen has frozen. The pragmatic priorities are the few things that keep patients safe: a written, rehearsed downtime procedure (how the practice consults on paper and reconciles afterwards), a known route to escalate to each supplier when a hosted system fails, and clarity over which SLAs those suppliers actually owe. A small independent health-tech supplier past the startup stage faces the mirror image — real customers depending on it but thin operational cover — and must offer credible support hours and incident response despite tight cash. The duty of care is identical to a large trust's; the resources are not, so the discipline is ruthless prioritization of the safety-critical basics over ITIL completeness.

### Enterprise

An NHS trust or large supplier runs service management in its fullest form: a formal service desk, ITIL-aligned incident/problem/change practices, change advisory boards, major-incident and business-continuity plans, and often a 24/7 operations center. The characteristic challenge is a sprawling legacy estate with weak configuration records, unpatched systems, and unclear ownership — the conditions that turned WannaCry into a crisis. The classic failures are change processes so heavy that staff bypass them, and "run" perpetually starved to fund the next build. The enterprise has the resources to do observability, sustainable on-call, and tested continuity properly, so the task is to keep the practices proportionate and genuinely clinically owned rather than a paperwork ritual that gives false assurance.
...
- **Change process theater.** A change board so heavy that teams route around it, or so light that risky changes ship unchecked. Antidote: right-size — pre-approve standard changes, reserve review for genuinely risky ones.
...
|---|---|---|---|---|---|
| Run funding & ownership | No run budget; services orphaned at go-live | Run funded ad-hoc; ownership unclear | Named owners; protected run budget for operational life | Run spend and ownership tracked against the estate; gaps reported and governed | Whole-life ownership; run and build balanced deliberately |
| Reliability targets | None; "up or down" only | Uptime measured but no agreed targets | SLIs/SLOs set, some with clinicians; SLAs defined | SLOs measured and reported against targets; breaches trigger owned action | Error budgets manage feature-vs-stability; SLOs clinically owned |
| Observability & alerting | Little visibility; users report outages first | Server monitoring only; noisy or ignored alerts | Logs/metrics/traces; symptom-based alerts on user experience | Reliability trends and alert quality reviewed on KPIs; coverage assured | Predictive detection; alerts tuned; near-misses caught early |
| Incident/problem/change | Firefighting; no post-incident learning | Incidents logged; root causes rarely fixed | Severity levels, blameless reviews, right-sized change control | Incident, problem and change metrics tracked; actions and change risk governed to closure | Systemic learning feeds design; recurrence steadily falls |
| Continuity & major incident | No tested downtime plan | Plans documented but never exercised | Downtime and major-incident plans rehearsed periodically | Exercise outcomes measured against readiness targets; gaps assured and closed | Continuity proven in real events; drills routine and improving |
