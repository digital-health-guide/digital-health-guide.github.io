# Chapter 1.5 — Regulation & Compliance Landscape
...
4. **Be proportionate and risk-based, not tick-box.** Regulation deserves the same proportionality as evidence (see Chapter 1.3): concentrate rigor where the risk to patients, equity, and trust is highest, and keep low-risk work light. Tick-box compliance — completing the form while missing the intent — is both wasteful and dangerous, because it manufactures false assurance. Aim to satisfy the *purpose* of each requirement, which usually also satisfies the letter.
...
3. **How would we know if a regulation that affects us is about to change — and who is responsible for turning that signal into planned work?**
   This probes horizon-scanning, the discipline that separates organizations surprised by deadlines from those that plan for them. It matters because the landscape is unusually live right now — UKCA and CE-marking timelines, the software-as-a-medical-device regime, the phased EU AI Act, evolving NHS instruments — and a rule change you knew was coming but did not prepare for is a self-inflicted wound that can pull a service off the market. The tension is that horizon-scanning has no immediate payoff, so it is the first thing dropped under delivery pressure, and its absence is invisible until the deadline arrives. Explore who currently tracks the relevant regulators and consultations, whether there is a watchlist tied to your compliance map, and how a signal becomes a ticket with an owner and a date rather than a worry in someone's inbox. Consider whether you would even hear about a change affecting only Northern Ireland or EU-facing parts of your service. A good answer names the person or role that owns regulatory horizon-scanning, points to a concrete watchlist, and describes the last time an upcoming change was turned into planned work ahead of the deadline.

4. **Have we honestly classified what we are — a medical device or not, a controller or a processor — or have we quietly chosen the interpretation that lowers our burden?**
   This forces the most consequential decision in the chapter into the open, because classification drives every downstream obligation and is the easiest place to deceive yourself. It matters because the incentives all pull one way: calling the product "a wellness app" rather than Software as a Medical Device, or "a processor" rather than a controller, dodges cost, evidence, and scrutiny — but if the algorithm interprets data to inform clinical action, or you decide the purposes of processing, the honest classification is not optional and the regulator will apply it regardless of your preference. The tension is that the cheaper reading is often defensible enough to survive an internal conversation but not an MHRA or ICO inquiry, and mis-classification is the fastest route to enforcement and a collapsed NHS pipeline. Work it concretely: state what the product actually does, take legal advice rather than a founder's hunch, and test whether your stated class would survive a regulator reading the same facts. A good, honest answer shows a documented classification reached early with advice, names who signed it off, and is candid about any borderline call and how it is being kept under review as the product changes.

5. **When one change to our service touches several regulators at once, do our compliance functions see a single shared picture — or three disconnected ones?**
   This tests coordination, the failure the chapter warns about when clinical safety, information governance, and product each hold a partial view. It matters because a single service genuinely spans bodies — a UX change to a triage app can alter its clinical risk profile, a fix that satisfies the MHRA can open a data-protection question, and a DTAC requirement can pull on all of them — so a siloed structure means no one owns the whole compliance position and changes fall between teams. The tension is that specialist functions are organized by discipline for good reasons, yet the service is experienced by the patient as one thing, and reconciling those views costs coordination effort that no single team is incentivized to fund. Explore whether there is one shared compliance map per service or several private ones, whether a change in one domain reliably triggers review in the others, and how this ties into board-level governance (Chapter 6.4). A good answer points to a single coordinated view of each service, describes a recent change that was assessed across all affected regulators together, and is honest about where disciplines still operate in silos.

6. **Where our service depends on a third-party supplier, do we know exactly which duties are theirs and which remain ours — or are we assuming "they're regulated, so we're covered"?**
   This surfaces the shared-responsibility gap that mis-classification and diffuse ownership both feed, because buying a regulated component does not transfer your own duties. It matters because a supplier may hold the medical-device conformity marking while you, as the deploying organization, still owe a DCB0160 clinical safety case, remain the data controller for your patients, and answer to the CQC for how the service is run — and the seam between "their compliance" and "yours" is exactly where obligations get dropped. The tension is commercial as well as regulatory: suppliers prefer contracts that imply they carry the burden, and it is tempting to accept that comfort rather than map the boundary precisely. Work through one real supplier relationship: list what the contract and their marking actually cover, what remains your statutory or mandatory duty, and whether your register records that split with named owners on your side. A good, honest answer distinguishes the supplier's obligations from your retained ones in writing, names who owns each retained duty, and can show the evidence you hold rather than the evidence you assume the supplier holds.
...
An established small provider — a GP practice, a community pharmacy, a care home, a single-site clinic, or a small health-tech supplier past the startup stage — carries the same statutory and mandatory duties as a large trust but without a Data Protection Officer, a Clinical Safety Officer, or a compliance team to discharge them. The DSPT still has to be completed every year, patient data still attracts the special-category bar, and any deployed clinical software still pulls in DCB0160, yet these usually land on a practice manager or owner already running operations. The realistic move is to make the compliance map deliberately small but real, lean on shared or commissioned expertise — a federation, a CSU, or a supplier's clinical-safety documentation — rather than pretend to hold every specialism in-house, and be scrupulous about the duties that cannot be delegated, such as remaining the data controller for your own patients. Treating a supplier's assurance as covering your retained duties is the characteristic trap here: buy in help, but keep named ownership of what is legally yours.

### Enterprise

An NHS trust or large ICS holds many services at once, so its challenge is portfolio-scale coordination: dozens of systems, each touching several regulators, with obligations that must not fall between teams. The failure mode is organizational silos — clinical safety, information governance, and procurement each holding a partial view, so no one sees the whole compliance position of a given service. The corrective is a shared compliance map per service, single-name accountability tied into board-level governance (Chapter 6.4), and continuous audit-ready evidence so a CQC "well-led" review or an ICO inquiry is answered from a live position. A large health-tech or pharma enterprise brings mature regulatory-affairs machinery but must guard against treating it as a compliance factory disconnected from how products are actually built.
...
- **Tick-box compliance.** Completing the DSPT or a safety case to pass, while the underlying risk is unmanaged. Antidote: satisfy the purpose, not just the form; test whether the artifact actually shaped the design.
...
- **Silomized regulators.** Clinical safety, IG, and product holding disconnected views of one service. Antidote: one shared compliance map, coordinated governance.
...
|---|---|---|---|---|---|
| Classification | Assumed or avoided | Done late, informally | Formal, early, legally reviewed | Class recorded, its basis assured and reviewed on a set cadence | Revisited as the product changes |
| Compliance map & register | None | Ad hoc list for a business case | Live register, owners, evidence, review dates | Register status tracked against KPIs, gaps reported to governance | Integrated with delivery and board governance |
| Accountability | Diffuse / unclear | Some named roles | Single-name owner per obligation | Owner performance measured, discharge of duties assured | Owners resourced, empowered, audited |
| Proportionality | Tick-box or absent | One-size rigor | Risk-based, calibrated to harm | Rigor-to-risk calibration measured and reviewed against outcomes | Explicit, defensible, continuously tuned |
| Horizon-scanning | Reactive to deadlines | Occasional awareness | Owned watchlist | Watchlist reviewed on cadence, response times tracked to deadlines | Signals routinely become planned work |
...
- [ ] Rigor is calibrated to patient, equity, and trust risk — not uniform box-ticking.
...
- [ ] Compliance functions share one coordinated view of each service, tied into governance (Chapter 6.4).
