...
**[Prioritization](https://en.wikipedia.org/wiki/Prioritization)** orders a backlog or portfolio; it is distinct from triage (which assessed and routed each item in isolation). Prioritization compares items *against each other* for finite capacity, and re-does that comparison as reality changes.
...
- **[MoSCoW](https://en.wikipedia.org/wiki/MoSCoW_method)** (Must / Should / Could / Won't-this-time), from [DSDM](https://en.wikipedia.org/wiki/Dynamic_systems_development_method): a categorization, excellent for scoping a release or a fixed-date program, weaker as a fine-grained ordering.
...
5. **Match the method to the constraint: capacity or date.** For a sustainable flow of change, order by value and pull work to a known capacity. For an immovable date, work backwards and use MoSCoW to protect the "Musts" and flex the "Coulds." Confusing the two — trying to fit fixed scope into fixed time and fixed capacity — is the classic route to a failed program.
...
8. **Guard against gaming and false precision.** Relative scores (Fibonacci, t-shirt sizes) are estimates, not truths; a WSJF of 8.3 is not meaningfully better than 7.9. Use the numbers to *inform* a ranked conversation, watch for inflated inputs, and let experienced judgment override an obviously perverse score — while recording why.
...
5. **Are we honest about which work is capacity-bound and which is date-bound — or are we trying to fix scope, date, and capacity all at once?**
   Prioritization must match the constraint: a sustainable flow of change is ordered by value and pulled to a known capacity, while an immovable regulatory go-live is worked backwards from the date with MoSCoW protecting the "Musts" and flexing the "Coulds." The classic route to a failed program is confusing the two — locking scope, date, and capacity simultaneously — and it is especially seductive in health and care, where a statutory deadline feels non-negotiable and stakeholders assume the full scope is equally non-negotiable. The team should debate, item by item, whether each major piece of work is really capacity-bound or date-bound, because treating a date-driven program as if it had elastic time, or a business-as-usual flow as if it had a hard deadline, mis-prioritizes everything downstream. Concrete angles: take an immovable go-live and ask which requirements are genuinely "Must" for the legal minimum versus "Should" or "Could" that have been smuggled into the mandatory set; look for the program where all three constraints are quietly fixed and name what will actually give when reality bites. A good answer classifies work by its binding constraint and applies the matching method deliberately, keeping capacity-based flow and date-based delivery as separate disciplines. The unrealistic answer promises full scope, on the fixed date, within fixed capacity, and hopes.
...
- **Framework theater.** Elaborate scoring produces a number nobody acts on. *Fix:* let the score inform a real, acted-on ranked order.
