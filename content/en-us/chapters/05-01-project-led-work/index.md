...
**In one sentence:** Project-led work is the right tool when the job has a defined scope, a finite life, and a clear end state — a temporary, controlled endeavor delivered to agreed time, cost, and quality, then closed.
...
3. **Manage by stages with real decision gates.** Break the work into management stages and hold a genuine stop/proceed review at each boundary. A gate that always says "proceed" is theater; the discipline only works if stopping or re-planning is a real option the board will exercise.
...
An acute trust must replace its aging pathology laboratory information management system before the incumbent supplier withdraws support in 14 months. This is textbook project-led work: fixed end date, defined scope, a hard contractual cliff-edge, and acute patient-safety risk if results are lost in migration.
