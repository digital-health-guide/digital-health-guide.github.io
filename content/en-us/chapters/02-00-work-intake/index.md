...
**[Demand management](https://en.wikipedia.org/wiki/Demand_management)**, in the [ITIL](https://en.wikipedia.org/wiki/ITIL) (IT Infrastructure Library) [service-management](https://en.wikipedia.org/wiki/IT_service_management) tradition, is the practice of anticipating and channeling demand so it can be met "without incurring unplanned costs." Its central device is the **single front door**: one route through which all requests are collected, so nothing is lost or processed through unofficial channels. ITIL 4 frames this as a [service catalog](https://en.wikipedia.org/wiki/Service_catalog) (the front door itself) plus service request management (what happens once someone walks through it).
...
- **Mandates** — statutory, regulatory, or nationally-directed work (e.g. an NHS England national program requirement) with limited discretion.
...
**Shadow intake** — the demand-side cousin of [shadow IT](https://en.wikipedia.org/wiki/Shadow_IT) — is the anti-pattern: demand that bypasses the front door — the corridor conversation, the direct email to a friendly developer, the "quick favor." Shadow intake is invisible, ungoverned, and the single biggest threat to a fair, safe system.
...
5. **Ask for just enough of a [business case](https://en.wikipedia.org/wiki/Business_case) — proportionate to size.** For small requests, a sentence of expected benefit is enough. For larger bids, use a lightweight one-page case: problem, intended outcome and who benefits, rough size, key risks, and the cost of *not* doing it. Reserve full business-case rigor (Green Book / Treasury five-case model) for genuine investment decisions, not for every idea.

6. **Set and publish an acknowledgment SLA.** Commit to a [service-level agreement](https://en.wikipedia.org/wiki/Service-level_agreement) for *acknowledgment* — for example, "every request acknowledged within two working days, with an indication of next steps and expected triage date." Acknowledgment is not agreement to do the work; it is the promise that the request has been seen and will be assessed. This single discipline does more than anything else to kill shadow intake, because people use back channels mainly when the front door feels like a black hole.
...
- **Heavyweight gate at the door.** Demanding a full business case to *submit* an idea suppresses good early signals and drives shadow intake. *Fix:* proportionate capture; rigor comes later.
- **Executive bypass.** Leaders route their own requests around the process, signaling that the front door is for other people. *Fix:* visible executive compliance; no exceptions.
...
- [ ] A published acknowledgment SLA exists and is measured.
