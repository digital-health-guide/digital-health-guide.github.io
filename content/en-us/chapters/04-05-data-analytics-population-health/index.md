...
4. **Run an explicit analyst and data-engineering operating model.** Separate the work of engineering reliable, well-modeled data pipelines from the work of analysing and interpreting. Adopt shared definitions, version control, peer review of analytical code, and reproducible methods so that a number means the same thing to everyone and can be regenerated on demand. The English "Reproducible Analytical Pipelines" movement in the public sector is a practical model worth borrowing.
...
3. **If a journalist asked how we use patients' data for analytics and research, could we give a clear, honest, reassuring answer today?** Public trust is the license on which all secondary use of health data depends, and it is easier to lose than to earn. Work through the actual answer: where does the data sit, who can access it, through what approval, for what purposes, and how would a member of the public find that out. If the honest answer involves copies of identifiable data scattered across spreadsheets and shared drives, you have both a security problem and a trust problem. Contrast that with a Secure Data Environment approach, where access is controlled, audited, and publishable. Discuss what your organization would need to change to be genuinely comfortable with public scrutiny — before an incident forces the question. Remember care.data: the technology was not the point of failure; the failure was public confidence in how data would be used.

4. **When we reach for a predictive model, can we actually act on what it tells us — and have we earned the right to trust the descriptive layer beneath it?** It is tempting to equate analytical maturity with sophistication, and to treat "AI-driven" or predictive work as the destination. But a risk score that identifies patients you have no capacity to contact is an academic exercise, and a predictive model built on a shaky descriptive foundation simply automates existing errors at speed. Interrogate each proposed model against two tests: is there a real decision that genuinely needs foresight rather than a reliable rear-view picture, and does the operational capacity exist to act on the prediction in time to change an outcome? An honest answer names the specific intervention the prediction will trigger and the team that will deliver it, and it is candid about whether the descriptive and diagnostic rungs are trustworthy enough to build on. Climbing the ladder deliberately, rung by rung, usually delivers more value than leaping to the top and discovering nothing catches the fall.

5. **Do we agree what our core numbers actually mean — and where would two teams give different answers to the same question?** As self-service analytics spreads, the risk shifts from having too few numbers to having too many versions of each one: the "waiting list", the "avoidable admission", the "active patient" quietly diverging across dashboards until meetings argue about whose figure is right rather than what to do. Test this concretely by asking two teams to produce the same headline metric independently and comparing not just the answer but the definition and the query behind it. The honest conversation surfaces where a governed semantic layer and certified datasets are missing, and where central definitions have failed to keep pace with local invention. The trade-off is real — clamp down too hard and you strangle the exploration that makes self-service valuable; leave it ungoverned and insight fragments into disagreement. A mature answer describes which metrics are certified and owned centrally and which are deliberately left open for local exploration, rather than pretending the whole estate can be one or the other.

6. **Are we fixing data quality where it is created, or endlessly correcting it downstream?** Every dashboard and model inherits the flaws of the operational records beneath it, so the most consequential analytics decisions are often made far upstream — in how a condition is coded at the point of care, or whether the same patient and service resolve to a single identifier. Discuss where your organization currently patches quality in the reporting layer, and what it would take to fix those problems at source instead, recognizing that downstream correction is both endless and lossy. This is inseparable from master data management: without a single authoritative version of patients, staff, and organizations, totals never reconcile and the same person fractures across records. The equity stakes are highest for the fields that are hardest to code well — ethnicity, deprivation, disability — where sparse or inconsistent capture quietly distorts every conclusion downstream. An honest answer names who owns quality for each critical flow, whether that ownership reaches into the operational systems, and where the absence of trustworthy reference data is silently undermining the numbers.
...
An established small provider — a GP practice, a community pharmacy, a care home, a single-site clinic — usually has real patients and statutory duties but no dedicated analyst or data team, so the practical goal is a trustworthy descriptive picture, not a maturity ladder. Lean on the analytics built into the systems you already run and on trusted national or ICB-provided datasets and dashboards rather than standing up a platform you cannot maintain. Fix data quality at source in your own operational coding, because that is the one lever fully within your control and it is what feeds every regional model of your population. When you must share data for population health or research, use the ICB's or a national Secure Data Environment rather than emailing extracts, which keeps you compliant with the same duties as large organizations without needing their resources. A single well-chosen metric that you actually act on — a list of patients overdue a review — beats a suite of reports no one has time to open.
...
- **Predictive theater.** Building sophisticated models the organization has no capacity to act on, or that quietly encode bias against the populations most in need.
- **Moving faster than trust.** Pursuing analytical ambition without public engagement, and losing the social license that all secondary data use depends on.
...
|---|---|---|---|---|---|
| Analytics ladder | Ad hoc spreadsheets; mostly manual descriptive reporting | Consistent descriptive dashboards; some diagnostic drill-down | Diagnostic standard; predictive used where it changes decisions | Predictive use governed and monitored; model performance and decision impact measured against targets | Predictive and prescriptive embedded where they demonstrably add value |
| Data platform & quality | Fragmented sources; quality unmeasured | Central warehouse or lake emerging; quality measured in places | Governed platform; data-quality metrics owned and published | Quality thresholds set and assured; platform and pipeline performance actively managed against agreed service levels | Modern platform with automated quality monitoring and reproducible pipelines |
| Governance & access | Identifiable extracts copied ad hoc | Access rules exist but inconsistently applied | Secure Data Environment as default route; access audited | SDE access controls assured and monitored; access requests and audit findings managed against defined standards | SDE mature, transparent to the public, access published |
| Population health & equity | No population view; reactive only | Basic segmentation; equity looked at occasionally | Risk stratification with routine equity breakdowns | Equity metrics tracked over time; targeting and bias checks governed and managed against agreed goals | Bias-audited targeting demonstrably narrowing inequalities |
| Value & literacy | Success measured by report volume | Some usage tracking; pockets of data literacy | Products linked to decisions; literacy program in place | Product value measured against targets; retirement and literacy managed as a governed, recurring cycle | Outcomes and value tracked; unused products retired; literacy widespread |
...
- [ ] A data-literacy program supports leaders and clinicians, not only analysts.
